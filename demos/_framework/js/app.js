...
(function () {
  ...
  function buildHeader(cfg) {
    ...
    var nav = el("nav", { class: "main-nav", id: "mainNav" }, [
  ...
  function buildHero(cfg) {
    ...
    hero.appendChild(el("div", { class: "scroll-cue" }, [ "Scroll", el("div", null, "↓") ]));
    return hero;
  }
  ...
  function buildGallery(cfg) {
    ...
    g.forEach(function (src, i) {
      grid.appendChild(el("div", { class: "gallery-item reveal", "data-index": i }, [
        el("img", { src: imgUrl(src, cfg), alt: cfg.business.name + " photo " + (i + 1), loading: "lazy" })
      ]));
    });

    // Lightbox
    var lbImg = el("img", { alt: "Enlarged" });
    var lb = el("div", { class: "lightbox", id: "lightbox" }, [
      el("button", { class: "lightbox__close", "aria-label": "Close" }, "×"),
      el("button", { class: "lightbox__nav lightbox__prev", "aria-label": "Previous" }, "‹"),
      lbImg,
      el("button", { class: "lightbox__nav lightbox__next", "aria-label": "Next" }, "›")
    ]);
    function showLb(i) {
    ...
    lb.querySelector(".lightbox__next").addEventListener("click", function () { showLb((lb._idx + 1) % g.length); });
    lb.querySelector(".lightbox__prev").addEventListener("click", function () { showLb((lb._idx - 1 + g.length) % g.length); });
  ...
  function buildContact(cfg) {
    ...
    var success = el("div", { class: "form-success", id: "formSuccess" }, [
  ...
  function field(name, label, type, placeholder, width) {
    var input;
    if (type === "textarea") {
      input = el("textarea", { name: name, id: name, placeholder: placeholder, rows: "4" });
    } else {
      input = el("input", { type: type, name: name, id: name, placeholder: placeholder });
    }
    return el("div", { class: "form-group" + (width === "full" ? " form-group--full" : "") }, [
      el("label", { for: name }, label),
      input,
      el("span", { class: "form-error" }, "Please enter a valid " + label.toLowerCase())
    ]);
  }
  ...
  function buildCookieConsent(cfg) {
    ...
    var banner = el("div", { class: "cookie-banner", id: "cookieBanner" });
